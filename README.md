<!-- 项目总览文档：说明 基于多模态感知的独居老人居家智能监护系统的功能、硬件、固件、Web 面板和部署方式。 -->
# 基于多模态感知的独居老人居家智能监护系统设计 （ESP32-S3）
...
- 系统按 ESP32-S3-N16R8 重新分配引脚，避开 GPIO22~25 以及常与 Flash/PSRAM 冲突的 GPIO26~37；MQ135、MQ7、FSR402 使用 ESP32-S3 ADC 可用的低号 GPIO。
...
- 暗环境夜灯模式：光照低于阈值时可开启基础照明，默认关闭，避免覆盖人体经过和起夜联动。
- 暗环境人体经过亮灯：光照低于阈值且 PIR 检测到人体活动时，自动打开 LED 灯并保持一段时间。
- 任意安全告警：强制打开 LED 灯。
...
| FanOn | int | 风扇状态 |
| LedOn | int | 灯光状态 |
| DarkLightOn | int | 暗环境直接亮灯状态 |
| NightLightOn | int | 暗环境有人亮灯状态 |
| NightWakeLightOn | int | 起夜离床亮灯状态 |
| AlarmLightOn | int | 告警强制亮灯状态 |
...
网页端还提供独立联动开关：暗环境夜灯模式、暗环境人体经过亮灯、起夜记录统计、起夜联动开灯、告警强制开灯、气体异常通风、蜂鸣器告警、SOS 舵机动作、无人活动提醒。直接双击打开 `web/public/index.html` 时会进入本地演示模式；真实实时数据建议使用 `http://59.110.166.166`。
